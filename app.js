...
const Icon = ({ name, size = 20, className = "" }) => {
    const paths = {
        ...
        calendar: "M19 4H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2zm0 0V2m-14 2V2", 
        ...
        lock: "M19 11H5a2 2 0 0 0-2 2v7a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7a2 2 0 0 0-2-2zm-7-7a4 4 0 0 1 4 4v3H8V8a4 4 0 0 1 4-4z", 
        shield: "M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z", 
        userCheck: "M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2 M16 11l2 2 4-4", 
        userX: "M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2 M18 8l5 5 M23 8l-5 5", 
        clock: "M12 22a10 10 0 1 0 0-20 10 10 0 0 0 0 20zM12 6v6l4 2", 
        tag: "M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z", 
        check: "M20 6L9 17l-5-5", 
        sparkles: "M12 2l3 6 6 3-6 3-3 6-3-6-6-3 6-3z", 
        ...
        merge: "M12 5v14M8 9l4-4 4 4", 
        link: "M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71",
        wallet: "M20 12V8H6a2 2 0 0 1-2-2c0-1.1.9-2 2-2h12v4",
        wallet2: "M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z", // Fallback generic
    ...
    // Map 'tag' to a wallet shape if wallet is missing, otherwise use tag
    const p = paths[name] || paths[name==='shopping-bag'?'shoppingBag':'tag'] || paths['tag'];
    return <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><path d={p} /></svg>;
};
...
const App = () => {
    ...
    const data = useMemo(() => {
        const groups = {};
        let monthTotal = 0; // Used for "Month Spending" (only expenses)
        let dailyTotal = 0; // Used for "Today" (only expenses)
        let totalIncome = 0;
        ...
        expenses.forEach(e => {
            ...
            const isIncome = e.type === 'income'; // Check new field
            ...
            if (isIncome) {
                totalIncome += amt;
                // Note: We usually don't add income to the "Monthly Spending" group total in the list UI, 
                // but for now let's keep the list showing everything.
                // Optional: Subtract income from group total? 
                // groups[k].total += amt; // Adds positive income to monthly summary
            } else {
                totalExpensesAllTime += amt;
                groups[k].total += amt; // Add to monthly expense total shown on list header
                
                // Monthly / Daily Stats (Expenses Only)
        ...
        // No category analytics needed for income usually, so keep it simple for now
        const analytics = []; // (Simplifying for brevity, add back if needed)
    ...
    const addTransaction = async () => { 
        await addDoc(collection(db, 'users', user.uid, 'expenses'), { 
            ...
            description: desc, 
            category: type === 'income' ? 'income_source' : cat, // Default cat for income or selected
            date, 
    ...
    return (
        <div className="container mx-auto px-4 pb-32 max-w-3xl">
                ...
                <div className="fixed inset-0 bg-black bg-opacity-80 flex items-end sm:items-center justify-center p-0 sm:p-4 z-50 animate-fade-in">
                    <div className="glass-panel w-full max-w-lg rounded-t-3xl sm:rounded-3xl p-8 animate-slide-up bg-[#0f172a]">
                        ...
                        <div className="flex bg-gray-800 p-1 rounded-xl mb-6">
                            ...
                            <button onClick={()=>setType('income')} className={`flex-1 py-3 rounded-lg font-bold transition ${type==='income' ? 'bg-emerald-600 text-white shadow' : 'text-gray-400 hover:text-white'}`}>Credit / Income</button>
                        </div>
            ...
            {isAdmin && <AdminPanel onClose={()=>setIsAdminOpen(false)} />}
            <SettingsPanel onClose={()=>setIsSettingsOpen(false)} user={user} categories={categories} isAdmin={isAdmin}/>
        </div>
    );
};
